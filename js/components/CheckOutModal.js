 ...
 *   with a conditional M-Pesa Code input
  ...
  return `
    ...
    <div class="flex-1 overflow-y-auto px-5 py-5 space-y-5">
      ...
      <div>
        ...
        <label class="block text-xs font-semibold text-gray-400 mb-1.5" for="co-payment-method">Payment Method</label>
        ...
        <div id="co-mpesa-code-wrap" class="mt-3 hidden">
          <label class="block text-xs font-semibold text-gray-400 mb-1.5" for="co-mpesa-code">M-Pesa Code</label>
          <input id="co-mpesa-code" type="text" placeholder="e.g. QFT3X9ZK1A"
            class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white
 ...
 * @param {Function} onCheckOut  - Called with { payment_method, mpesa_code } when checkout button clicked.
 */
function _wireCheckOutPanel(room, onAddShop, onCheckOut) {
  _selectedPaymentMethod = "cash";

  const paymentSelect = document.getElementById("co-payment-method");
  const mpesaWrap = document.getElementById("co-mpesa-code-wrap");
  const mpesaInput = document.getElementById("co-mpesa-code");
  const validationMsg = document.getElementById("co-validation-msg");

  // Payment method toggling
  paymentSelect.addEventListener("change", (e) => {
    _selectedPaymentMethod = e.target.value;
    mpesaWrap.classList.toggle("hidden", _selectedPaymentMethod !== "mpesa");
  ...
  document.getElementById("btn-checkout")?.addEventListener("click", () => {
    ...
    if (paymentMethod === "mpesa" && !mpesaInput.value.trim()) {
      validationMsg.textContent = "Enter the M-Pesa code to continue.";
    ...
    onCheckOut({
      ...
      mpesa_code: paymentMethod === "mpesa" ? mpesaInput.value.trim() : null
    });
  });
}
 ...
 *   onCheckOut: Function,   // ({ payment_method, mpesa_code }) => void
 * }} callbacks
 */
